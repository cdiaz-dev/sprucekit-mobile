...
fun BitmapImage(
    ...
    fun convertImageByteArrayToBitmap(imageData: ByteArray): Bitmap? { // Return nullable Bitmap
        val bitmap = BitmapFactory.decodeByteArray(imageData, 0, imageData.size)
        if (bitmap == null) {
            // Log an error or handle the failure case
            Log.e("UtilsKt", "Bitmap decoding failed for byte array of size: ${imageData.size}")
        }
        return bitmap
    }
